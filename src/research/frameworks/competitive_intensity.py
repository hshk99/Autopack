...
class CompetitiveIntensity:
    ...
    def add_competitor(
        self, name: str, market_share: float, strengths: List[str], weaknesses: List[str]
    ) -> None:
    ...
    def get_market_concentration(self) -> Dict[str, Any]:
        ...
        if not self.competitors:
            return {"herfindahl_index": 0.0, "top_3_share": 0.0, "concentration_level": "Unknown"}

        # Calculate Herfindahl-Hirschman Index (HHI)
        hhi = sum(comp["market_share"] ** 2 for comp in self.competitors)

        # Calculate top 3 market share
        sorted_competitors = sorted(self.competitors, key=lambda x: x["market_share"], reverse=True)
