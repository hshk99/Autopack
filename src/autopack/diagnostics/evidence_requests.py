...
class EvidenceRequestBuilder:
    ...
    def request_error(
        self, description: str, priority: EvidencePriority = EvidencePriority.HIGH
    ) -> "EvidenceRequestBuilder":
    ...
    def request_command_output(
        self, command: str, description: str, priority: EvidencePriority = EvidencePriority.MEDIUM
    ) -> "EvidenceRequestBuilder":
    ...
    def request_file(
        self, description: str, path: str, priority: EvidencePriority = EvidencePriority.MEDIUM
    ) -> "EvidenceRequestBuilder":
    ...
    def request_env_var(
        self, var_name: str, description: str, priority: EvidencePriority = EvidencePriority.LOW
    ) -> "EvidenceRequestBuilder":
...
def create_evidence_request_for_failure(
    phase_id: str, failure_type: str, failure_message: str, context: Optional[Dict[str, Any]] = None
    ...
    if failure_type in ("test_failure", "test_error"):
        builder.request_test_output(
            "Full test output with failures", priority=EvidencePriority.CRITICAL
        )
        builder.request_log(
        ...
        builder.request_error(
            "Full build/compilation error output", priority=EvidencePriority.CRITICAL
        )
        ...
        builder.request_command_output(
            "pip list", "Installed packages list", priority=EvidencePriority.HIGH
        )
        builder.request_file(
            "Requirements file", "requirements.txt", priority=EvidencePriority.MEDIUM
        )

    elif failure_type in ("git_apply_failed", "patch_error"):
        builder.request_command_output(
            "git status", "Current git status", priority=EvidencePriority.HIGH
        )
        builder.request_command_output(
            "git diff", "Current uncommitted changes", priority=EvidencePriority.MEDIUM
        )

    elif failure_type in ("database_error", "connection_error"):
        builder.request_config("Database configuration", priority=EvidencePriority.HIGH)
        ...
        builder.request_error(
            f"Full error details for: {failure_message[:100]}", priority=EvidencePriority.CRITICAL
        )
        builder.request_log("Relevant log output", priority=EvidencePriority.HIGH)
