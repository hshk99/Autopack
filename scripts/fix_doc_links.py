    ...
    for source_file_rel, fixes in sorted(fixes_by_file.items()):
        ...
        for fix in sorted(fixes, key=lambda f: f['line_number'], reverse=True):
            ...
            if fixed_line != original_line:
                if dry_run:
                    print(f"📝 {source_file_rel}:{line_num} [{confidence}]")
...
def main():
    ...
    if not args.execute:
        print()
        print("ℹ️  This was a dry-run. Add --execute to apply fixes.")
        print("   Example: python scripts/fix_doc_links.py --execute")
    else:
        print()
        print("✅ Fixes applied. Re-run check_doc_links.py to validate.")
