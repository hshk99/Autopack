...
class TestResearchAPIQuarantine:
    ...
    def test_research_router_has_quarantine_guard(self):
        ...
        router_file = (
            REPO_ROOT / "src" / "autopack" / "research" / "api" / "router.py"
        )
        ...
        assert "research_guard" in content, (
            "Research router must have research_guard decorator"
        )
...
class TestResearchEndpointsHaveGuard:
    ...
    def test_get_sessions_has_guard(self):
        ...
        assert "@research_guard" in content.split("def get_research_sessions")[0].split(
            "@research_router.get"
        )[-1], "get_research_sessions must have @research_guard decorator"
    ...
    def test_create_session_has_guard(self):
        ...
        assert "@research_guard" in content.split("def create_research_session")[0].split(
            "@research_router.post"
        )[-1], "create_research_session must have @research_guard decorator"
...
class TestResearchQuarantineDocumentation:
    ...
    def test_router_has_docstring(self):
        ...
        assert content.strip().startswith('"""'), (
            "Research router must have module docstring"
        )
