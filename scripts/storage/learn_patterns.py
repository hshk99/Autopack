...
def analyze_patterns(args):
    ...
    try:
        ...
        patterns = analyzer.analyze_approval_patterns(
            category=args.category, max_patterns=args.max_patterns
        )
        ...
        for i, pattern in enumerate(patterns, 1):
            ...
            print(
                f"   Confidence: {pattern.confidence:.1%} ({pattern.approvals} approvals, {pattern.rejections} rejections)"
            )
...
def list_rules(args):
    ...
    try:
        ...
        rules = query.order_by(
            LearnedRule.confidence_score.desc(), LearnedRule.created_at.desc()
        ).all()
        ...
        for rule in rules:
            ...
            print(
                f"   Evidence: {rule.based_on_approvals} approvals, {rule.based_on_rejections} rejections"
            )
...
def get_recommendations(args):
    ...
    try:
        ...
        recommendations = engine.generate_recommendations(
            max_recommendations=args.max_recommendations, lookback_days=args.lookback_days
        )
...
def run_workflow(args):
    ...
    analyze_args = argparse.Namespace(
        category=None, max_patterns=10, min_samples=5, min_confidence=0.75, non_interactive=True
    )
    ...
    rec_args = argparse.Namespace(max_recommendations=10, lookback_days=90, verbose=False)
    ...
    print(
        "  2. Approve/reject rules: python scripts/storage/learn_patterns.py approve <rule_id> --by <your_name>"
    )
...
def main():
    ...
    analyze_parser.add_argument(
        "--max-patterns", type=int, default=10, help="Maximum patterns to detect"
    )
    analyze_parser.add_argument(
        "--min-samples", type=int, default=5, help="Minimum approval samples"
    )
    analyze_parser.add_argument(
        "--min-confidence", type=float, default=0.75, help="Minimum confidence threshold"
    )
    analyze_parser.add_argument(
    ...
    list_parser.add_argument(
        "--status",
        type=str,
        choices=["pending", "approved", "rejected", "applied"],
    ...
    recommend_parser.add_argument(
        "--max-recommendations", type=int, default=10, help="Maximum recommendations"
    )
    recommend_parser.add_argument(
        "--lookback-days", type=int, default=90, help="Days of history to analyze"
    )
