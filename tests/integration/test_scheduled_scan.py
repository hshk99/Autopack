...
class TestScheduledScanSetup:
    ...
    def test_create_scheduled_task_fails_without_admin(self, mock_run):
        ...
        mock_run.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd=["schtasks"], stderr="ERROR: Access is denied."
        )
...
class TestScheduledScanWorkflow:
    ...
    @patch(
        "autopack.storage_optimizer.telegram_notifications.StorageTelegramNotifier.send_scan_completion"
    )
...
class TestWizTreePerformance:
    ...
    @pytest.mark.slow
    @pytest.mark.skipif(
        not Path("C:/Program Files/WizTree/wiztree64.exe").exists(), reason="WizTree not installed"
    )
    def test_wiztree_scan_faster_than_python(self):
        ...
        print(
            f"WizTree: {wiztree_duration:.2f}s, Python: {python_duration:.2f}s, Speedup: {speedup:.1f}x"
        )
