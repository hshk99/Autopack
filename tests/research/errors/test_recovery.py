...
class TestRecovery:
    ...
    def test_retry_mechanism(self):
        ...
        with patch("requests.get", side_effect=failing_request):
            result = collector.collect(query="test", sources=["http://example.com"])
    ...
    def test_partial_failure_handling(self):
        ...
        with patch("requests.get", side_effect=mixed_response):
            result = collector.collect(
                query="test",
                sources=["http://success1.com", "http://fail.com", "http://success2.com"],
            )
    ...
    def test_graceful_degradation(self):
        ...
        with patch("requests.get", side_effect=Exception("All sources down")):
            data = collector.collect(
                query="test", sources=["http://source1.com", "http://source2.com"]
            )
    ...
    def test_circuit_breaker_pattern(self):
        ...
        with patch("requests.get", side_effect=failing_service):
            ...
            for i in range(5):
                try:
                    collector.collect(query="test", sources=["http://failing-service.com"])
                except Exception:
                    pass
