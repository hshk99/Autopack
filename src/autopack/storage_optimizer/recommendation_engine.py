...
class RecommendationEngine:
    ...
    def __init__(self, db: Session, policy: StoragePolicy, min_scans_for_trends: int = 3):
        """
    ...
    def generate_recommendations(
        self, max_recommendations: int = 10, lookback_days: int = 90
    ) -> List[Recommendation]:
        """
        ...
        scans = (
            self.db.query(StorageScan)
            .filter(StorageScan.timestamp >= cutoff_date)
            .order_by(StorageScan.timestamp.asc())
            .all()
        )
        ...
        recommendations.sort(
            key=lambda r: (priority_order[r.priority], -(r.potential_savings_bytes or 0))
        )
                ...
                recommendations.append(
                    Recommendation(
                        ...
                        evidence={
                            ...
                            "size_30_days_ago_gb": round(
                                trend["earliest_size_bytes"] / (1024**3), 2
                            ),
                        },
                        action=f"Review and clean up {category} more frequently, or tighten retention policy",
                        potential_savings_bytes=int(
                            monthly_growth_gb * 0.7 * 1024**3
                        ),  # 70% of monthly growth
                    )
                )
    ...
    def _detect_recurring_waste(self, scans: List[StorageScan]) -> List[Recommendation]:
        ...
        for scan in scans:
            candidates = (
                self.db.query(CleanupCandidateDB)
                .filter(
            ...
            if len(occurrences) >= min(3, len(scans) // 2):
                ...
                recommendations.append(
                    Recommendation(
                        ...
                        potential_savings_bytes=int(
                            avg_size * 12
                        ),  # Annual savings if cleaned monthly
                    )
                )
    ...
    def _identify_top_consumers(self, scans: List[StorageScan]) -> List[Recommendation]:
        ...
        category_sizes = (
            ...
            .filter(CleanupCandidateDB.scan_id == latest_scan.id)
            .group_by(CleanupCandidateDB.category)
            .order_by(func.sum(CleanupCandidateDB.size_bytes).desc())
            .all()
        )
    ...
    def _analyze_category_growth(self, scans: List[StorageScan]) -> Dict[str, Dict]:
        ...
        for scan in scans:
            category_sizes = (
                ...
                .filter(CleanupCandidateDB.scan_id == scan.id)
                .group_by(CleanupCandidateDB.category)
                .all()
            )
            ...
            for category, size in category_sizes:
                ...
                category_trends[category]["data_points"].append(
                    {"timestamp": scan.timestamp, "size_bytes": size}
                )
    ...
    def _analyze_rejection_patterns(self, scans: List[StorageScan]) -> Dict[str, Dict]:
        """Analyze user rejection patterns by category."""
        rejection_stats = defaultdict(
            lambda: {
                "rejected_count": 0,
                "approved_count": 0,
                "total_count": 0,
