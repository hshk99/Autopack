    ...
    def test_record_bucket_switch(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            telemetry.record_bucket_switch(
                run_id="test-run",
                phase_id="phase-1",
                files=[("file1.py", 700), ("file2.py", 800)]
            )
...
class TestIntegration:
    ...
    def test_telemetry_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            telemetry.record_preflight_reject(
                "run-1", "phase-1", "file1.py", 1200, 1000, "C"
            )
            telemetry.record_bucket_switch(
                "run-1", "phase-1", [("file2.py", 700)]
            )
            telemetry.record_shrinkage(
                "run-1", "phase-2", "file3.py", 1000, 300, 70.0, False
            )
