...
class TestEvidenceValidator:
    ...
    def test_validate_evidence_with_citations(self, validator):
        ...
        citation = Citation(
            source_url="https://example.com", title="Test Source", accessed_at=datetime.now()
        )
        evidence = Evidence(
    ...
    def test_validate_evidence_without_citations(self, validator):
        ...
        with pytest.raises(ValueError):
            Evidence(content="Uncited claim", citations=[], quality=EvidenceQuality.HIGH)
    ...
    def test_validate_evidence_quality_threshold(self, validator):
        ...
        low_quality = Evidence(
            content="Low quality evidence", citations=[citation], quality=EvidenceQuality.LOW
        )
    ...
    def test_validate_evidence_content_length(self, validator):
        ...
        short_evidence = Evidence(
            content="Too short", citations=[citation], quality=EvidenceQuality.HIGH
        )
...
class TestCitationValidator:
    ...
    def test_validate_url_format(self, validator):
        ...
        with pytest.raises(ValueError):
            Citation(source_url="not-a-url", title="Test", accessed_at=datetime.now())

    def test_validate_url_accessibility(self, validator):
        """Test checking if URL is accessible."""
        citation = Citation(
            source_url="https://example.com/nonexistent", title="Test", accessed_at=datetime.now()
        )
    ...
    def test_validate_citation_freshness(self, validator):
        ...
        old_citation = Citation(
            source_url="https://example.com", title="Old Article", accessed_at=datetime(2020, 1, 1)
        )
...
class TestQualityValidator:
    ...
    def test_validate_insufficient_evidence(self, validator):
        ...
        citation = Citation(
            source_url="https://example.com", title="Single Source", accessed_at=datetime.now()
        )

        evidence = Evidence(
            content="Single piece of evidence", citations=[citation], quality=EvidenceQuality.MEDIUM
        )
    ...
    def test_validate_source_diversity(self, validator):
        ...
        evidence_items = [
            Evidence(
                content=f"Evidence {i}", citations=[citations[i]], quality=EvidenceQuality.HIGH
            )
            for i in range(5)
        ]
...
class TestValidationFramework:
    ...
    def test_framework_runs_all_validators(self, framework):
        ...
        evidence = Evidence(
            content="Test evidence", citations=[citation], quality=EvidenceQuality.HIGH
        )
        report = ResearchReport(
            query="Test", summary="Summary", evidence=[evidence], conclusions=["Conclusion"]
        )
