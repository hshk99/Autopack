...
class ErrorHandler:
    ...
    def __init__(self, max_retries: int = 3, backoff_factor: float = 2.0):
        ...
        logger.info(
            f"ErrorHandler initialized with max_retries={max_retries}, backoff_factor={backoff_factor}"
        )
    ...
    def handle_error(self, exception: Exception, context: Optional[str] = None) -> None:
        ...
        if isinstance(exception, requests.exceptions.HTTPError):
            ...
            logger.error(
                f"Unexpected error{context_str}: {type(exception).__name__} - {str(exception)}"
            )
