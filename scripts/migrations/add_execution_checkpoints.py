...
def create_execution_checkpoints_table(session, db_type):
    ...
    if db_type == 'postgresql':
        ...
        # Create indexes
        session.execute(text("CREATE INDEX idx_execution_checkpoints_run_id ON execution_checkpoints(run_id)"))
        session.execute(text("CREATE INDEX idx_execution_checkpoints_candidate_id ON execution_checkpoints(candidate_id)"))
        session.execute(text("CREATE INDEX idx_execution_checkpoints_sha256 ON execution_checkpoints(sha256)"))
        session.execute(text("CREATE INDEX idx_execution_checkpoints_timestamp ON execution_checkpoints(timestamp DESC)"))
        session.execute(text("CREATE INDEX idx_execution_checkpoints_status ON execution_checkpoints(status)"))
...
def main():
    ...
    try:
        ...
        print("\nExample query:")
        print("  SELECT * FROM execution_checkpoints WHERE status = 'failed' ORDER BY timestamp DESC LIMIT 10;")
