...
def test_readme_sot_summary_matches_derived_state():
    ...
    decisions_match = re.search(
        r'- \*\*Architecture Decisions\*\*:\s*(\d+)',
        summary_block
    )
    debug_match = re.search(
        r'- \*\*Debugging Sessions\*\*:\s*(\d+)',
        summary_block
    )
    ...
    readme_build_unique = int(builds_match.group(2)) if builds_match.group(2) else readme_build_entries
    ...
    # Derive build counts
    table_rows = re.findall(r'^\|\s*\d{4}-\d{2}-\d{2}\s*\|\s*BUILD-\d+', build_history, re.MULTILINE)
    ...
    derived_latest_build = f"{latest_build_id}: {title}{status_suffix}" if title else f"{latest_build_id}{status_suffix}"
    ...
    debug_table_rows = re.findall(r'^\|\s*(\d{4}-\d{2}-\d{2})\s*\|\s*(DBG-\d+|\[?\d{4}-\d{2}-\d{2}\]?|[^|]+)', debug_log, re.MULTILINE)
    if debug_table_rows:
        valid_rows = [row for row in debug_table_rows if 'DBG-' in row[1] or re.match(r'\[?\d{4}-\d{2}-\d{2}\]?', row[1].strip())]
        derived_debug_count = len(valid_rows)
    else:
        dbg_ids = set(re.findall(r'\bDBG-\d+\b', debug_log))
        date_headers = set(re.findall(r'^#{2,3}\s+\[?(\d{4}-\d{2}-\d{2})\]?', debug_log, re.MULTILINE))
