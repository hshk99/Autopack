...
class TestFullResearchPipeline:
    ...
    async def test_simple_research_query_end_to_end(self, orchestrator):
        ...
        query = ResearchQuery(
            query="What is Python?", context={"depth": "basic"}, constraints={"max_sources": 5}
        )
    ...
    async def test_pipeline_with_constraints(self, orchestrator):
        ...
        query = ResearchQuery(
            ...
            context={},
            constraints={"max_sources": 3, "time_limit": 60, "min_quality": "high"},
        )
    ...
    async def test_pipeline_error_recovery(self, orchestrator):
        ...
        query = ResearchQuery(query="Test query", context={}, constraints={})

        session = await orchestrator.create_session(query)

        # Simulate error in one stage
        with patch.object(
            orchestrator, "_execute_source_discovery", side_effect=Exception("Network error")
        ):
            result = await orchestrator.execute_pipeline(session.session_id)
    ...
    async def test_concurrent_research_sessions(self, orchestrator):
        ...
        queries = [ResearchQuery(query=f"Query {i}", context={}, constraints={}) for i in range(3)]
        ...
        results = await asyncio.gather(
            *[orchestrator.execute_pipeline(s.session_id) for s in sessions], return_exceptions=True
        )

        # All should complete
        assert len(results) == 3
        assert all(
            r.status in ["completed", "error"] for r in results if not isinstance(r, Exception)
        )
    ...
    async def test_validation_integration(self, orchestrator):
        ...
        query = ResearchQuery(
            query="What is machine learning?", context={}, constraints={"min_evidence_count": 5}
        )
