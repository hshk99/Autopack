...
class TestClassificationEndpoints:
    ...
    def test_classify_with_pack(self):
        ...
        test_data = {
            "text": "CANADA REVENUE AGENCY T4 (2024)",
            "pack_id": "canada_documents"
        }
...
class TestRateLimiting:
    ...
    def test_rate_limit_not_exceeded(self):
        ...
        response = client.get("/health")
        assert response.status_code == 200
        assert "X-RateLimit-Remaining" not in response.headers or \
               int(response.headers.get("X-RateLimit-Remaining", "1")) >= 0
