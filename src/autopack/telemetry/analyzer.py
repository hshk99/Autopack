...
@dataclass
class InsightAccuracyReport:
    """Report measuring accuracy of insight predictions vs actual outcomes (IMP-LOOP-035).

    Tracks how well TelemetryAnalyzer's insights predict actual problems,
    enabling recalibration of anomaly detection thresholds.

    Attributes:
        insight_id: Unique identifier for the insight being measured.
        predicted_problem: The problem predicted by the insight.
        actual_problem: The actual problem observed after task execution.
        prediction_correct: Whether the prediction matched the actual outcome.
        predicted_confidence: The confidence level of the original prediction.
        confidence_calibration: Calibration factor for threshold adjustment.
            Values < 1.0 indicate overconfidence, > 1.0 indicate underconfidence.
        measured_at: Timestamp when accuracy was measured.
        task_id: Optional associated task ID if insight led to task generation.
        notes: Additional notes about the measurement.
    """
    ...
    measured_at: datetime = None  # type: ignore[assignment]
...
class ThresholdUpdate:
    """Represents an update to anomaly detection thresholds (IMP-LOOP-035).

    Generated by recalibrate_thresholds() when accuracy feedback indicates
    that current thresholds are over- or under-sensitive.

    Attributes:
        threshold_name: Name of the threshold being updated.
        old_value: Previous threshold value.
        new_value: Updated threshold value.
        calibration_factor: Factor applied to calculate new value.
        reason: Explanation for the threshold change.
        accuracy_reports_count: Number of accuracy reports used for calibration.
        updated_at: Timestamp when threshold was updated.
    """
...
class TelemetryAnalyzer:
    ...
    def __init__(self, db_session: Session, memory_service=None):
        ...
        # IMP-LOOP-035: Insight accuracy tracking for threshold recalibration
        self._insights: Dict[str, Dict[str, Any]] = {}  # insight_id -> insight data
        self._insight_accuracy_reports: List[InsightAccuracyReport] = []
        self._current_thresholds: Dict[str, float] = {
            "cost_sink_threshold": 50000.0,  # Token threshold for cost sink detection
            "failure_rate_threshold": 0.3,  # Failure rate threshold for anomaly
            "retry_threshold": 3,  # Retry count threshold
            "latency_threshold_ms": 30000.0,  # Latency threshold in milliseconds
        }
    ...
    # IMP-LOOP-035: Insight accuracy measurement methods
    ...
    def register_insight(
        ...
        """Register an insight for later accuracy measurement.

        IMP-LOOP-035: Stores insight predictions so they can be compared
        against actual outcomes when the related task completes.
        ...
        Args:
            ...
            phase_id: Optional phase ID related to the insight.
            phase_type: Optional phase type related to the insight.
            metric_value: The metric value that triggered this insight.
            metadata: Additional metadata for the insight.
        """
        ...
        logger.debug(
            "[IMP-LOOP-035] Registered insight for accuracy tracking: "
            "insight_id=%s, predicted_problem=%s, confidence=%.2f",
            insight_id,
            predicted_problem[:50] if predicted_problem else "None",
            confidence,
        )

    def get_insight(self, insight_id: str) -> Optional[Dict[str, Any]]:
        """Retrieve a registered insight by ID.

        IMP-LOOP-035: Returns the insight data for accuracy measurement.

        Args:
            insight_id: The insight identifier to retrieve.

        Returns:
            Insight data dictionary if found, None otherwise.
        """
        return self._insights.get(insight_id)

    def get_outcome_for_insight(
        self,
        insight_id: str,
        task_id: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Get the actual outcome data for an insight.

        IMP-LOOP-035: Queries telemetry to determine the actual outcome
        for a given insight, enabling comparison against the prediction.
        ...
        Returns:
            ...
            - actual_problem: The actual problem observed
            - success: Whether the related phase/task succeeded
            - outcome_details: Additional outcome information
        """
        insight = self._insights.get(insight_id)
        if not insight:
            return {
                "actual_problem": "unknown",
                "success": False,
                "outcome_details": {"error": "Insight not found"},
            }
        ...
        # Query telemetry for actual outcomes
        cutoff = insight.get("registered_at", datetime.now(timezone.utc)) - timedelta(hours=1)

        # Build query based on available identifiers
        if phase_id:
            result = self.db.execute(
                text("""
                    SELECT phase_outcome, stop_reason, tokens_used, duration_seconds
                    FROM phase_outcome_events
                    ...
                    LIMIT 1
                """),
                {"phase_id": phase_id, "cutoff": cutoff},
            )
        elif phase_type:
            result = self.db.execute(
                text("""
                    SELECT phase_outcome, stop_reason, tokens_used, duration_seconds
                    FROM phase_outcome_events
                    WHERE phase_type = :phase_type
                    AND timestamp >= :cutoff
                    ORDER BY timestamp DESC
                    LIMIT 1
                """),
                {"phase_type": phase_type, "cutoff": cutoff},
            )
        else:
            return {
                "actual_problem": "unknown",
                "success": False,
                "outcome_details": {"error": "No phase identifier available"},
            }

        row = result.fetchone()
        if not row:
            return {
                "actual_problem": "no_outcome_data",
                "success": False,
                "outcome_details": {"error": "No outcome data found"},
            }

        # Determine actual problem based on outcome
        phase_outcome = row.phase_outcome
        stop_reason = row.stop_reason
        tokens_used = row.tokens_used or 0
        duration_seconds = row.duration_seconds or 0

        actual_problem = self._determine_actual_problem(
            issue_type=issue_type,
    ...
    def _determine_actual_problem(
        ...
        """Determine the actual problem based on outcome data.

        IMP-LOOP-035: Maps telemetry outcomes to problem classifications.

        Args:
            issue_type: Original predicted issue type.
            phase_outcome: The phase outcome (SUCCESS, FAILURE, etc).
            stop_reason: Reason for stopping if not successful.
            tokens_used: Number of tokens used.
            duration_seconds: Duration of the phase.

        Returns:
            String describing the actual problem.
        """
        # Check for cost sink (high token usage)
        if tokens_used > self._current_thresholds.get("cost_sink_threshold", 50000):
            return "cost_sink_confirmed"

        # Check for failure mode
        if phase_outcome != "SUCCESS":
            if stop_reason and "timeout" in stop_reason.lower():
                return "timeout_failure"
            elif stop_reason and "error" in stop_reason.lower():
                return "error_failure"
            else:
                return f"failure_{stop_reason}" if stop_reason else "generic_failure"

        # Check for latency issues
        latency_threshold_ms = self._current_thresholds.get("latency_threshold_ms", 30000)
        if duration_seconds * 1000 > latency_threshold_ms:
            return "high_latency"
    ...
    def _calculate_calibration_factor(
        ...
        """Calculate calibration factor for threshold adjustment.

        IMP-LOOP-035: Computes how much to adjust confidence/thresholds
        based on whether the prediction was correct.

        Args:
            predicted_confidence: The original confidence level.
            prediction_correct: Whether the prediction was accurate.

        Returns:
            Calibration factor: <1.0 means overconfident, >1.0 means underconfident.
        """
        if prediction_correct:
            # Prediction was correct - confidence was appropriate or underconfident
            # Calibration factor > 1.0 suggests we could be more confident
            if predicted_confidence < 0.5:
                return 1.2  # We were underconfident
            elif predicted_confidence < 0.8:
                return 1.1  # Slightly underconfident
            else:
                return 1.0  # Confidence was appropriate
        else:
            # Prediction was wrong - we were overconfident
            if predicted_confidence > 0.8:
                return 0.7  # Significantly overconfident
            elif predicted_confidence > 0.5:
                return 0.85  # Moderately overconfident
            else:
                return 0.95  # Only slightly overconfident (low confidence anyway)

    def measure_insight_accuracy(
        self,
        insight_id: str,
        task_id: Optional[str] = None,
    ) -> InsightAccuracyReport:
        """Measure accuracy of insight prediction vs actual outcome.

        IMP-LOOP-035: Compares the predicted problem from an insight against
        the actual outcome observed in telemetry.

        Args:
            insight_id: The insight identifier to measure.
            task_id: Optional associated task ID.

        Returns:
            InsightAccuracyReport with accuracy metrics.
        """
        insight = self.get_insight(insight_id)
        if not insight:
            # Return a report indicating insight not found
            return InsightAccuracyReport(
                insight_id=insight_id,
                predicted_problem="unknown",
                actual_problem="insight_not_found",
                prediction_correct=False,
                predicted_confidence=0.0,
                confidence_calibration=1.0,
                task_id=task_id,
                notes="Insight not found in registry",
            )

        # Get actual outcome
        outcome = self.get_outcome_for_insight(insight_id, task_id)

        predicted_problem = insight.get("predicted_problem", "unknown")
        actual_problem = outcome.get("actual_problem", "unknown")
        predicted_confidence = insight.get("confidence", 0.5)

        # Determine if prediction was correct
        # Exact match or related problem types count as correct
        prediction_correct = self._is_prediction_correct(predicted_problem, actual_problem)

        # Calculate calibration factor
        calibration = self._calculate_calibration_factor(predicted_confidence, prediction_correct)
        ...
        report = InsightAccuracyReport(
            ...
            task_id=task_id,
            notes=f"Issue type: {insight.get('issue_type', 'unknown')}",
        )

        # Store the report
        self._insight_accuracy_reports.append(report)

        logger.info(
            "[IMP-LOOP-035] Measured insight accuracy: insight_id=%s, "
            "predicted=%s, actual=%s, correct=%s, calibration=%.2f",
            insight_id,
            predicted_problem[:30] if predicted_problem else "None",
            actual_problem[:30] if actual_problem else "None",
            prediction_correct,
            calibration,
        )
    ...
    def _is_prediction_correct(self, predicted: str, actual: str) -> bool:
        """Determine if prediction matches actual problem.

        IMP-LOOP-035: Uses fuzzy matching to determine if the prediction
        was essentially correct, even if not an exact match.

        Args:
            predicted: The predicted problem string.
            actual: The actual problem string.
        ...
        # Check for related problem types
        problem_families = {
            "cost": ["cost_sink", "high_token", "token_usage", "cost_sink_confirmed"],
            "failure": ["failure", "error", "failed", "timeout_failure", "error_failure"],
            "timeout": ["timeout", "time_out", "timeout_failure"],
            "latency": ["latency", "slow", "high_latency", "delay"],
            "retry": ["retry", "retries", "retry_cause"],
        }

        # Find which family the predicted problem belongs to
        predicted_family = None
        for family, keywords in problem_families.items():
            if any(kw in predicted_lower for kw in keywords):
                predicted_family = family
                break

        # Check if actual problem is in the same family
        if predicted_family:
            family_keywords = problem_families.get(predicted_family, [])
            if any(kw in actual_lower for kw in family_keywords):
                return True

        # Check for no-problem cases
        no_problem_indicators = ["no_problem", "success", "no_issue", "none"]
        predicted_no_problem = any(ind in predicted_lower for ind in no_problem_indicators)
        actual_no_problem = any(ind in actual_lower for ind in no_problem_indicators)

        if predicted_no_problem and actual_no_problem:
            return True
    ...
    def recalibrate_thresholds(
        self,
        accuracy_reports: Optional[List[InsightAccuracyReport]] = None,
        min_reports: int = 5,
    ) -> List[ThresholdUpdate]:
        """Adjust anomaly detection thresholds based on accuracy feedback.

        IMP-LOOP-035: Uses accumulated accuracy reports to recalibrate
        detection thresholds. This closes the feedback loop by making
        the insight generation more accurate over time.

        Args:
            accuracy_reports: List of InsightAccuracyReport to use. If None,
                uses internally stored reports.
            min_reports: Minimum number of reports required for recalibration.

        Returns:
            List of ThresholdUpdate objects describing changes made.
        """
        if accuracy_reports is None:
            accuracy_reports = self._insight_accuracy_reports

        if len(accuracy_reports) < min_reports:
            logger.info(
                "[IMP-LOOP-035] Insufficient reports for recalibration: "
                "%d reports, need at least %d",
                len(accuracy_reports),
                min_reports,
            )
            return []

        # Calculate average calibration factor
        calibration_factors = [r.confidence_calibration for r in accuracy_reports]
        avg_calibration = sum(calibration_factors) / len(calibration_factors)

        # Calculate accuracy rate
        correct_count = sum(1 for r in accuracy_reports if r.prediction_correct)
        accuracy_rate = correct_count / len(accuracy_reports)

        updates: List[ThresholdUpdate] = []

        # Recalibrate thresholds based on accuracy and calibration
        for threshold_name, current_value in self._current_thresholds.items():
            # Determine if this threshold needs adjustment
            should_adjust, adjustment_factor, reason = self._should_adjust_threshold(
            ...
            if should_adjust:
                ...
                update = ThresholdUpdate(
                    ...
                    new_value=new_value,
                    calibration_factor=adjustment_factor,
                    reason=reason,
                    accuracy_reports_count=len(accuracy_reports),
                )
                updates.append(update)

                # Apply the update
                self._current_thresholds[threshold_name] = new_value

                logger.info(
                    "[IMP-LOOP-035] Recalibrated threshold: %s from %.2f to %.2f "
                    "(factor=%.2f, reason=%s)",
                    threshold_name,
                    current_value,
                    new_value,
        ...
        if updates:
            logger.info(
                "[IMP-LOOP-035] Recalibrated %d thresholds based on %d accuracy reports "
                "(accuracy_rate=%.2f%%, avg_calibration=%.2f)",
                len(updates),
                len(accuracy_reports),
                accuracy_rate * 100,
                avg_calibration,
            ...
            logger.info(
                "[IMP-LOOP-035] No threshold recalibration needed "
                "(accuracy_rate=%.2f%%, avg_calibration=%.2f)",
                accuracy_rate * 100,
                avg_calibration,
            )
    ...
    def _should_adjust_threshold(
        self,
        threshold_name: str,
        accuracy_rate: float,
        avg_calibration: float,
        accuracy_reports: List[InsightAccuracyReport],
    ) -> tuple:
        """Determine if a threshold should be adjusted and by how much.

        IMP-LOOP-035: Analyzes accuracy data to determine if threshold
        adjustment is warranted.

        Args:
            threshold_name: Name of the threshold.
            accuracy_rate: Overall accuracy rate of predictions.
            avg_calibration: Average calibration factor.
            accuracy_reports: List of accuracy reports for analysis.

        Returns:
            Tuple of (should_adjust: bool, factor: float, reason: str).
        """
        # Filter reports relevant to this threshold type
        threshold_type_map = {
            "cost_sink_threshold": ["cost", "token"],
            "failure_rate_threshold": ["failure", "error"],
            "retry_threshold": ["retry"],
            "latency_threshold_ms": ["latency", "slow"],
        }

        keywords = threshold_type_map.get(threshold_name, [])
        relevant_reports = [
            r
            for r in accuracy_reports
            if any(kw in r.predicted_problem.lower() for kw in keywords)
            or any(kw in r.actual_problem.lower() for kw in keywords)
        ]

        if not relevant_reports:
            return (False, 1.0, "No relevant reports")

        # Calculate relevant metrics
        relevant_calibrations = [r.confidence_calibration for r in relevant_reports]
        relevant_avg_calibration = sum(relevant_calibrations) / len(relevant_calibrations)
        relevant_correct = sum(1 for r in relevant_reports if r.prediction_correct)
        relevant_accuracy = relevant_correct / len(relevant_reports)

        # Determine adjustment
        # Low accuracy with high calibration = threshold too sensitive (increase)
        # Low accuracy with low calibration = threshold too lenient (decrease)
        if relevant_accuracy < 0.5:
            if relevant_avg_calibration < 0.9:
                # Overconfident and inaccurate - make threshold more lenient
                return (True, 1.2, f"Low accuracy ({relevant_accuracy:.0%}), overconfident")
            else:
                # Underconfident but inaccurate - make threshold stricter
                return (True, 0.9, f"Low accuracy ({relevant_accuracy:.0%}), adjust sensitivity")

        elif relevant_accuracy > 0.8:
            # High accuracy - only adjust if significantly miscalibrated
            if relevant_avg_calibration < 0.8:
                return (True, 1.1, "High accuracy but overconfident, slight adjustment")
            elif relevant_avg_calibration > 1.2:
                return (True, 0.95, "High accuracy but underconfident, slight adjustment")

        return (False, 1.0, "Threshold performing well")

    def get_insight_accuracy_summary(self) -> Dict[str, Any]:
        """Get a summary of insight accuracy tracking status.

        IMP-LOOP-035: Provides visibility into the accuracy tracking pipeline,
        showing how well insights predict actual problems.

        Returns:
            Dictionary containing:
            - total_insights: Number of registered insights
            - total_accuracy_reports: Number of accuracy measurements
            - overall_accuracy: Percentage of correct predictions
            - avg_calibration: Average calibration factor
            - accuracy_by_type: Breakdown by issue type
            - current_thresholds: Current threshold values
        """
        reports = self._insight_accuracy_reports
        ...
        if not reports:
            return {
                ...
                "avg_calibration": 1.0,
                "accuracy_by_type": {},
                "current_thresholds": dict(self._current_thresholds),
            }

        correct_count = sum(1 for r in reports if r.prediction_correct)
        overall_accuracy = correct_count / len(reports)
        avg_calibration = sum(r.confidence_calibration for r in reports) / len(reports)

        # Group by issue type (from notes)
        accuracy_by_type: Dict[str, Dict[str, Any]] = {}
        for report in reports:
            # Extract issue type from notes
            issue_type = "unknown"
            if "Issue type:" in report.notes:
                issue_type = report.notes.split("Issue type:")[1].strip()

            if issue_type not in accuracy_by_type:
                accuracy_by_type[issue_type] = {"total": 0, "correct": 0}

            accuracy_by_type[issue_type]["total"] += 1
            if report.prediction_correct:
                accuracy_by_type[issue_type]["correct"] += 1

        # Calculate accuracy rate for each type
        for issue_type, stats in accuracy_by_type.items():
            stats["accuracy"] = stats["correct"] / stats["total"] if stats["total"] > 0 else 0.0

        return {
            "total_insights": len(self._insights),
            "total_accuracy_reports": len(reports),
    ...
    def get_current_thresholds(self) -> Dict[str, float]:
        ...
        IMP-LOOP-035: Returns the current threshold values, which may have
        been recalibrated based on accuracy feedback.

        Returns:
            Dictionary of threshold_name -> value.
        """
    ...
    def set_threshold(self, threshold_name: str, value: float) -> bool:
        ...
        IMP-LOOP-035: Allows manual threshold adjustment.
        ...
        Returns:
            True if threshold was set, False if threshold name is unknown.
        """
        if threshold_name not in self._current_thresholds:
            logger.warning(
                "[IMP-LOOP-035] Unknown threshold name: %s",
                threshold_name,
            )
            return False

        old_value = self._current_thresholds[threshold_name]
        self._current_thresholds[threshold_name] = value

        logger.info(
            "[IMP-LOOP-035] Manually set threshold: %s from %.2f to %.2f",
            threshold_name,
            old_value,
            value,
        )
