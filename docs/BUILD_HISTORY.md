...
<!-- META
Last_Updated: 2026-01-01T23:30:00Z
Total_Builds: 150
Format_Version: 2.0
Auto_Generated: False
Sources: CONSOLIDATED files, archive/, manual updates, BUILD-150 Tidy Archive Nesting Fix
-->
...
|-----------|----------|-------|---------|---------------|
| 2026-01-01 | BUILD-150 | Tidy Archive Nesting Fix (100% COMPLETE ✅) | **Self-Healing Archive Path Repair**: Fixed critical bug in tidy_workspace.py where `archive_dir = root / "archive"` was appended unconditionally, creating malformed `archive/archive/` nesting when root was already the archive directory. **Root Cause**: Line 525 `plan_non_md_actions()` didn't check if `root` already contained "archive" in path before appending, causing paths like `c:\dev\Autopack\archive` → `c:\dev\Autopack\archive\archive` → deeply nested `archive/archive/runs/general/diagnostics/logs/`. **Bug Fix**: Added conditional logic checking `root.name == "archive" or "archive" in root.parts` before appending (lines 525-532), preventing duplicate "archive" in path construction. **Self-Healing Mechanism**: Created `detect_malformed_paths()` (80 lines) detecting consecutive duplicate folder names and excessive "runs" nesting, created `apply_self_healing()` (35 lines) with dry-run default to automatically repair malformed paths, added `--self-heal` CLI flag for opt-in self-healing. **Validation**: Path validation already existed at lines 184-207 (`validate_destination_path()`) detecting duplicate nesting, but creation bug was upstream. **CLI Integration**: Self-healing runs before all tidy operations when `--self-heal` flag provided, reports number of paths repaired with detailed logging. **Documentation**: Added entry to BUILD_HISTORY.md documenting root cause (unconditional archive append), fix location (lines 525-532), self-healing mechanism design, production impact (makes Autopack truly reusable, not just this workspace). **Impact**: Archive nesting bug eliminated at source (no longer creates malformed paths), self-healing cleans up existing malformed paths from prior runs, Autopack truly reusable across workspaces (fix in code, not manual cleanup), workspace structural integrity maintained automatically. **Safety**: Dry-run default for self-healing (requires `--execute` to apply repairs), validation still blocks new malformed path creation, existing `collapse_consecutive_duplicates()` helpers reused for repair logic. Files: scripts/tidy/tidy_workspace.py (bug fix lines 525-532, self-healing functions lines 518-609, CLI integration lines 1301-1307), docs/BUILD_HISTORY.md (this entry) | 2 |
...
- SHA256: `5e0e91775ad4704961f2bebabbaa61a920ad11bffcc8215d848ab67fe96d9b84`