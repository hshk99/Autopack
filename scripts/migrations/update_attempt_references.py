...
def update_autonomous_executor():
    ...
    old_pattern = (
        r'f"\[{phase_id}\] Updated attempts in DB: {attempts_used}/{phase\.max_attempts} "'
    )
    ...
    if n > 0:
        changes.append(
            f"Replaced phase_db.attempts_used with phase_db.retry_attempt ({n} replacements)"
        )
    ...
    content, n = re.subn(
        r"max_attempts = phase_db\.max_attempts", "max_attempts = MAX_RETRY_ATTEMPTS", content
    )
    if n > 0:
    ...
    content, n = re.subn(
        r"attempt_index = phase_db\.attempts_used",
    ...
    content, n = re.subn(
        r"new_attempts = phase_db\.attempts_used \+ 1",
        "new_attempts = phase_db.retry_attempt + 1",
        content,
    )
    if n > 0:
    ...
    content, n = re.subn(
        r"attempts_used < max_attempts", "retry_attempt < MAX_RETRY_ATTEMPTS", content
    )
    if n > 0:
    ...
    content, n = re.subn(
        r"Database tracks: attempts_used, max_attempts",
        "Database tracks: retry_attempt, revision_epoch, escalation_level",
        content,
    )
    if n > 0:
    ...
    content, n = re.subn(
        r'"max_attempts": max_attempts', '"max_retry_attempts": MAX_RETRY_ATTEMPTS', content
    )
    if n > 0:
