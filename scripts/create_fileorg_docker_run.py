...
Exercises Autopack's ability to:
1. Author container configs (Dockerfile, docker-compose)
2. Wire deployment scripts/docs
3. Validate multi-service local deployment instructions
"""
...
# Generate unique run ID
RUN_ID = f"fileorg-docker-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
...
PHASES = [
    {
        ...
        "tier_id": "tier-docker",
        "name": "Docker Deployment",
        "description": """Create Docker deployment assets for FileOrganizer (Phase 2 Task 3).

Goals:
- Author backend Dockerfile (Python 3.11 + uvicorn + deps)
- Add docker-compose.yml orchestrating API + DB services
- Provide .dockerignore to keep images lean
- Add deploy.sh helper script for local builds
- Update DEPLOYMENT_GUIDE.md with container instructions
- Smoke test docker-compose workflow locally (document commands)
...
Acceptance Criteria:
- Dockerfile builds backend image with pinned dependencies
- docker-compose.yml includes API, Postgres/SQLite volume, and optional frontend
- .dockerignore excludes venv/, node_modules/, dist/, __pycache__/
- deploy.sh automates build/up/down lifecycle
- DEPLOYMENT_GUIDE.md documents prerequisites + commands
- docker-compose up works locally (document verification logs)
""",
        "task_category": "deployment",
        "complexity": "medium",
        "builder_mode": "infrastructure",
        "scope": {
            "paths": [
                ".autonomous_runs/file-organizer-app-v1/Dockerfile",
                ".autonomous_runs/file-organizer-app-v1/docker-compose.yml",
                ".autonomous_runs/file-organizer-app-v1/.dockerignore",
                ".autonomous_runs/file-organizer-app-v1/deploy.sh",
                ".autonomous_runs/file-organizer-app-v1/docs/DEPLOYMENT_GUIDE.md",
            ],
            "read_only_context": [
                ".autonomous_runs/file-organizer-app-v1/fileorganizer/backend/app/",
                ".autonomous_runs/file-organizer-app-v1/fileorganizer/backend/requirements.txt",
                ".autonomous_runs/file-organizer-app-v1/README.md",
                ".autonomous_runs/file-organizer-app-v1/docs/USER_GUIDE.md",
            ],
        },
        "ci": {
            "type": "pytest",
...
TIERS = [
    {
        ...
        "tier_index": 0,
        "name": "Docker Deployment",
        "description": "Containerize FileOrganizer backend + auxiliary services",
    }
]


def create_run():
    """Create run for FileOrganizer Docker deployment"""
    ...
    print(f"[INFO] Total phases: {len(PHASES)}\n")
    print("[INFO] This run will test Autopack's ability to:")
    print("  - Build deployment-ready Docker images")
    print("  - Compose multi-service stacks")
    print("  - Generate deployment scripts & docs\n")
    print("[INFO] Target: .autonomous_runs/file-organizer-app-v1/\n")
