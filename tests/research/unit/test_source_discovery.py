...
from src.research.intent_clarification import ClarifiedIntent
    ...
    async def test_discover_sources(self, strategy, sample_intent):
        ...
        with patch.object(strategy, '_execute_search', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                {"url": "https://example.com/1", "title": "API Design Guide", "snippet": "Best practices..."},
                {"url": "https://example.com/2", "title": "REST API Tutorial", "snippet": "Learn REST..."}
            ]
    ...
    async def test_source_ranking(self, strategy, sample_intent):
        ...
        with patch.object(strategy, '_execute_search', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                ...
                {"url": "https://example.com/2", "title": "Less relevant", "snippet": "General programming"}
            ]
...
class TestAcademicSearchStrategy:
    ...
    async def test_discover_academic_sources(self, strategy, academic_intent):
        ...
        with patch.object(strategy, '_search_academic_databases', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                {
...
class TestDocumentationSearchStrategy:
    ...
    async def test_discover_documentation(self, strategy, technical_intent):
        ...
        with patch.object(strategy, '_search_documentation_sites', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                {
    ...
    async def test_prioritize_official_docs(self, strategy, technical_intent):
        ...
        with patch.object(strategy, '_search_documentation_sites', new_callable=AsyncMock) as mock_search:
            mock_search.return_value = [
                {"url": "https://docs.python.org/3/library/asyncio.html", "title": "Official", "snippet": "..."},
                {"url": "https://realpython.com/async-io-python/", "title": "Tutorial", "snippet": "..."}
            ]
