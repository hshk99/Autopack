...
def scan_error_artifacts(error_dir: Path, since_date: Optional[datetime] = None) -> List[Dict]:
    """
...
def propose_rule(signature: str, occurrences: List[Dict], confidence: str = "low") -> Dict:
    """
    ...
    evidence = list(set(occ.get("run_id") for occ in occurrences if occ.get("run_id")))[
        :10
...
def append_rules(rules_path: Path, new_rules: List[Dict], dry_run: bool = True) -> None:
    """
    ...
    existing_signatures = {rule["signature"] for rule in existing_rules}

    # Filter new rules (only add if signature not present)
    rules_to_add = [rule for rule in new_rules if rule["signature"] not in existing_signatures]
...
def main():
    parser = argparse.ArgumentParser(description="Analyze failures and propose learned rules")
    parser.add_argument(
        "--error-dir",
        type=Path,
    ...
    parser.add_argument(
        "--max", type=int, default=25, help="Max number of top failures to report (default: 25)"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--output-report", type=Path, help="Optional: Export report to markdown file"
    )
    ...
    # Sort by frequency
    sorted_signatures = sorted(grouped.items(), key=lambda x: len(x[1]), reverse=True)[: args.max]
    ...
    if args.output_report:
        report_path = (
            ...
            else repo_root / args.output_report
        )
        report_lines = [
