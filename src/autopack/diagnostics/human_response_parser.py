...
class ParsedResponseBatch:
    ...
    def get_context_for_prompt(self, max_tokens: int = 2000) -> str:
        ...
        sorted_evidence = sorted(
            self.parsed_evidence, key=lambda e: (e.quality.value, e.request_index)
        )
...
class HumanResponseParser:
    ...
    def parse_response(
        self, response_text: str, original_requests: Optional[EvidenceRequestBatch] = None
    ) -> ParsedResponseBatch:
        ...
        result.total_token_estimate = sum(e.token_estimate for e in result.parsed_evidence)
    ...
    def _add_evidence_from_dict(
        self, index: int, item: Dict[str, Any], result: ParsedResponseBatch
    ) -> None:
    ...
    def _validate_against_requests(
        self, result: ParsedResponseBatch, requests: EvidenceRequestBatch
    ) -> None:
...
def format_response_for_context(response: HumanResponse) -> str:
    ...
    lines = [
        f"💬 HUMAN GUIDANCE [{response.phase_id}]",
...
def validate_response_for_decision(
    response: HumanResponse, num_options: int
...
def compress_evidence_for_prompt(
    evidence_items: List[Dict[str, str]], max_tokens: int = 1500
) -> str:
