...
def generate_do_not_build_report(build_decision: BuildDecision) -> str:
    """Generate a comprehensive "Do Not Build" report.

    Creates a detailed report explaining why the project should not be built
    based on research findings and viability analysis.

    Args:
        build_decision: BuildDecision with DO_NOT_BUILD decision

    Returns:
        Markdown-formatted report explaining the decision
    """
    metrics = build_decision.metrics
    report_lines = [
        "# Build Decision Report: NOT RECOMMENDED",
        "",
        "## Summary",
        "",
        f"**Decision**: {build_decision.decision.value}",
        f"**Confidence**: {build_decision.confidence_percentage:.0f}%",
        "",
        "## Decision Rationale",
        "",
        f"{build_decision.rationale}",
        "",
    ]
    ...
    report_lines.extend(
        [
            ...
            "| Metric | Score |",
            "|--------|-------|",
            f"| Market Attractiveness | {metrics.market_attractiveness_score:.1f}/10 |",
            f"| Competitive Intensity | {metrics.competitive_intensity_score:.1f}/10 |",
            f"| Technical Feasibility | {metrics.technical_feasibility_score:.1f}/10 |",
            f"| Overall Confidence | {metrics.overall_confidence} |",
    ...
    # Add key blockers
    if build_decision.key_blockers:
        report_lines.extend(
            [
                "## Critical Blockers",
                "",
        ...
        for blocker in build_decision.key_blockers:
            report_lines.append(f"- {blocker}")
        report_lines.append("")

    # Add recommendations
    if build_decision.recommended_mitigations:
        report_lines.extend(
            [
                "## Recommendations for Future Consideration",
                "",
                "Before reconsidering this project, address the following:",
                "",
            ]
        )
        for mitigation in build_decision.recommended_mitigations:
            report_lines.append(f"- {mitigation}")
        report_lines.append("")
    ...
    report_lines.extend(
        [
            "## Next Steps",
            "",
            "### Option 1: Re-evaluate After Changes",
            "Address the critical blockers identified above and re-run the research phase.",
            "",
            "### Option 2: Pivot or Adjust Scope",
            "Consider adjusting the project scope or business model to address market or feasibility concerns.",
            "",
            "### Option 3: Defer to Future Research Cycle",
            "Monitor market conditions and revisit when key blockers may have changed.",
            "",
            "---",
            "",
            "*Report generated by Autopack Research Analysis*",
        ]
    )
...
def generate_build_decision_adjusted_artifacts(build_decision: BuildDecision) -> Dict[str, Any]:
    """Generate artifact generation configuration based on build decision.

    Returns a configuration dict that guides which artifacts to generate
    and with what emphasis based on the build decision.

    Args:
        build_decision: BuildDecision from research analysis

    Returns:
        Config dict with generation strategy and artifact list
    """
    config = {
        "decision": build_decision.decision.value,
        "confidence": build_decision.confidence_percentage,
        "artifacts": [],
        "emphasis": [],
        "notes": [],
    }

    if build_decision.decision == BuildDecisionType.BUILD:
        # Generate full artifact suite
        config["artifacts"] = [
            "project_brief",
            "code",
            "deployment",
        ...
        config["emphasis"] = [
            ...
            "All artifacts included for comprehensive guidance",
        ]
    elif build_decision.decision == BuildDecisionType.BUILD_WITH_CAUTION:
        # Generate artifacts but with risk mitigation focus
        config["artifacts"] = [
            "project_brief",
            "code",
            "deployment",
        ...
        config["emphasis"] = [
            "Include mitigation strategies in all artifacts",
            "Prioritize MVP planning and phased rollout",
            "Add risk assessment sections to deployment guide",
        ]
        config["notes"] = build_decision.recommended_mitigations
    else:
        # DO_NOT_BUILD: Only generate the decision report
        config["artifacts"] = ["do_not_build_report"]
        config["emphasis"] = [
            "Decision report explains why not to build",
            "Recommendations provided for future reconsideration",
        ]
        config["notes"] = build_decision.key_blockers
...
class ArtifactGeneratorRegistry:
    ...
    def generate_with_build_decision(
        self,
        build_decision: BuildDecision,
        research_findings: Optional[Dict[str, Any]] = None,
        **kwargs: Any,
    ) -> Dict[str, str]:
        """Generate artifacts based on build decision.

        Routes artifact generation based on the build decision. For BUILD decisions,
        generates full artifact suite. For BUILD_WITH_CAUTION, includes mitigation
        strategies. For DO_NOT_BUILD, generates only the decision report.

        Args:
            build_decision: BuildDecision from research analysis
            research_findings: Optional research findings dict for artifact generation
            **kwargs: Additional arguments to pass to generators

        Returns:
            Dict mapping artifact names to their generated content

        Example:
            ```python
            decision = orchestrator.get_build_decision(session)
            artifacts = registry.generate_with_build_decision(decision, research_findings)
            for artifact_name, content in artifacts.items():
                # Save artifacts to files
            ```
        """
        artifacts = {}
        config = generate_build_decision_adjusted_artifacts(build_decision)

        logger.info(
            f"Generating artifacts for decision: {config['decision']} "
            f"(confidence: {config['confidence']:.0f}%)"
        )

        # Generate special "do_not_build_report" if needed
        if "do_not_build_report" in config["artifacts"]:
            report = generate_do_not_build_report(build_decision)
            artifacts["do_not_build_report"] = report
            logger.info("Generated 'Do Not Build' decision report")

        # For BUILD and BUILD_WITH_CAUTION, generate standard artifacts
        if research_findings:
            for artifact_type in config["artifacts"]:
                if artifact_type == "do_not_build_report":
                    # Already handled above
                    continue

                try:
                    generator = self.get(artifact_type, **kwargs)
                    if generator is None:
                        logger.warning(f"Generator not found for artifact type: {artifact_type}")
                        continue
                    ...
                    if hasattr(generator, "generate"):
                        ...
                        logger.debug(f"Generated artifact: {artifact_type}")
                    else:
                        logger.warning(f"Generator {artifact_type} has no generate method")
                except Exception as e:
                    logger.error(
                        f"Error generating {artifact_type} artifact: {e}",
                        exc_info=True,
                    )

        if config["notes"]:
            artifacts["_decision_notes"] = "\n".join(config["notes"])

        logger.info(
            f"Generated {len(artifacts)} artifacts for build decision: {config['decision']}"
        )
        return artifacts
