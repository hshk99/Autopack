...
class CalibrationReport:
    ...
    def calculate_overall_confidence(self) -> None:
        ...
        weighted_sum = sum(c.confidence * c.sample_count for c in self.changes)
...
class CalibrationReporter:
    ...
    def _calculate_confidence(self, sample_count: int, change_magnitude: float) -> float:
    ...
    def to_markdown(self, report: CalibrationReport) -> str:
        ...
        if report.changes:
            ...
            lines.append("")
            lines.append(
                "| Coefficient | Old Value | New Value | Change | Change % | Samples | Confidence |"
            )
            lines.append(
                "|-------------|-----------|-----------|--------|----------|---------|------------|"
            )
    ...
    def save_report(self, report: CalibrationReport, filename: Optional[str] = None) -> Path:
    ...
    def save_json(self, report: CalibrationReport, filename: Optional[str] = None) -> Path:
