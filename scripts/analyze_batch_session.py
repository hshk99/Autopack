...
def analyze_session(session_path: Path) -> None:
    ...
    if results:
        ...
        for run_id, run_results in runs.items():
            ...
            run_timeouts = sum(
                1 for r in run_results if r.get("subprocess_returncode") in [-1, 143]
            )
            ...
            print(
                f"  {run_id} {stopped}: {run_success}/{len(run_results)} success, {run_timeouts} timeouts, {run_events} events"
            )
    ...
    if session.get("completed_at"):
        ...
        if timeout_rate > 0.3:
            ...
            print(
                "   Consider: --phase-timeout-seconds 600 (reduce to 10 minutes for faster triage)"
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "session_id", nargs="?", help="Session ID (e.g., batch-drain-20251228-061426)"
    )
    ...
    if args.latest:
        ...
        if not session_path:
            print(
                "ERROR: No session files found in .autonomous_runs/batch_drain_sessions/",
                file=sys.stderr,
            )
            return 1
