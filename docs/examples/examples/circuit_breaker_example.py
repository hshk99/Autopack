...
def example_basic_usage():
    ...
    breaker = CircuitBreaker(
        name="basic_service", config=CircuitBreakerConfig(failure_threshold=3, timeout=2.0)
    )
...
def example_with_fallback():
    ...
    breaker = CircuitBreaker(
        name="fallback_service", config=CircuitBreakerConfig(failure_threshold=2)
    )
...
def example_registry():
    ...
    for name, service in services.items():
        registry.register(name, CircuitBreakerConfig(failure_threshold=3))
...
def example_monitoring():
    ...
    breaker = CircuitBreaker(
        name="monitored_service", config=CircuitBreakerConfig(failure_threshold=5)
    )
...
def example_custom_exception():
    ...
    breaker = CircuitBreaker(
        ...
        config=CircuitBreakerConfig(failure_threshold=3, expected_exception=ServiceError),
    )
