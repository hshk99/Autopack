...
class MetaAuditor:
    ...
    def assess_strategic_fit(self) -> Dict[str, Any]:
        ...
        if (
            self.framework_results.market_attractiveness
            and self.framework_results.product_feasibility
        ):
            market_level = self.framework_results.market_attractiveness.get(
                "attractiveness_level", ""
            )
            feasibility_level = self.framework_results.product_feasibility.get(
                "feasibility_level", ""
            )
        ...
        if (
            self.framework_results.competitive_intensity
            and self.framework_results.adoption_readiness
        ):
    ...
    def identify_key_opportunities(self) -> List[Dict[str, Any]]:
        ...
        if self.framework_results.market_attractiveness:
            ...
            tech_trends = self.framework_results.market_attractiveness.get("key_factors", {}).get(
                "technology_trends", []
            )
        ...
        if self.framework_results.competitive_intensity:
            positioning = self.framework_results.competitive_intensity.get(
        ...
        if self.framework_results.adoption_readiness:
            recommendations = self.framework_results.adoption_readiness.get(
    ...
    def generate_executive_summary(self) -> str:
        ...
        summary_parts.append(
            f"Strategic Recommendation: {recommendation.value.replace('_', ' ').title()}"
        )
        ...
        if self.framework_results.market_attractiveness:
            level = self.framework_results.market_attractiveness.get(
                "attractiveness_level", "unknown"
            )
        ...
        return {
            ...
            "opportunity_assessment": {
                ...
                "high_impact_opportunities": sum(
                    1 for o in opportunities if o.get("potential_impact") == "high"
                ),
