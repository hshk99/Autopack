-- Migration: Fix token_estimation_v2_events complexity constraint
-- Created: 2025-12-24
-- Purpose:
--   Fix complexity CHECK constraint to match actual codebase values.
--   Current: CHECK(complexity IN ('low', 'medium', 'high', 'critical'))
--   Correct: CHECK(complexity IN ('low', 'medium', 'high', 'maintenance'))
--
--   The codebase normalizes complexity with ALLOWED_COMPLEXITIES = {"low", "medium", "high", "maintenance"}
--   but the DB constraint uses 'critical' which doesn't exist in the codebase.
--
-- Approach (SQLite-safe):
--   Recreate token_estimation_v2_events with correct constraint.
...
-- Rebuild table with correct complexity constraint
