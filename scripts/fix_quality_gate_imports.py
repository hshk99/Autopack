...
def fix_quality_gate_refs():
    ...
    for i, line in enumerate(lines):
        ...
        if line_num == 44 and "from autopack.quality_gate import QualityGate" in line:
            modified.append(
                "# from autopack.quality_gate import QualityGate  # BUILD-126: Phase G not complete yet\n"
            )
        ...
        if line_num == 1030 and "self.quality_gate = QualityGate(" in line:
            modified.append(
                "            # self.quality_gate = QualityGate(repo_root=self.workspace)  # BUILD-126: Phase G not complete\n"
            )
        ...
        if line_num == 1031 and "Quality Gate: Initialized" in line:
            modified.append(
                '            # logger.info("Quality Gate: Initialized")  # BUILD-126: Phase G not complete\n'
            )
        ...
        if line_num == 4417 and "# Step 5: Apply Quality Gate" in line:
            ...
            modified.append(
                line.replace(
                    "# Step 5:",
                    "# BUILD-126: Step 5: Quality Gate disabled - Phase G not complete yet\n            # Step 5:",
                )
            )
        ...
        if in_quality_gate_block:
            ...
            if line_num >= 4473 and (
                current_indent <= block_indent or "# Update phase status to COMPLETE" in line
            ):
                in_quality_gate_block = False
