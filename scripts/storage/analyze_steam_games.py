...
def print_game_table(games: List[SteamGame], show_all: bool = False):
    ...
    for game in games:
        ...
        last_updated = (
            game.last_updated.strftime("%Y-%m-%d %H:%M") if game.last_updated else "Unknown"
        )
...
def main():
    ...
    parser.add_argument(
        "--min-size", type=float, default=10.0, help="Minimum game size in GB (default: 10.0)"
    )
    ...
    parser.add_argument("--all", action="store_true", help="Show all games regardless of size/age")
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed information (paths, app IDs)"
    )
    ...
    if args.all:
        ...
        games = detector.find_unplayed_games(min_size_gb=args.min_size, min_age_days=args.min_age)
