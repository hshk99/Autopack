...
def run_parallel_example(
    workers: int = 3, use_postgres: bool = True, runs_dir: str = None, cleanup: bool = True
):
    ...
    try:
        result = subprocess.run(
            cmd, cwd=repo_path, env={**subprocess.os.environ, "PYTHONUTF8": "1"}
        )
        ...
        if cleanup:
            ...
            cleanup_count = WorkspaceManager.cleanup_all_worktrees(
                repo=repo_path, worktree_base=Path(runs_dir) / "workspaces"
            )
...
def main():
    ...
    # Configuration
    parser.add_argument(
        "--workers", type=int, default=3, help="Number of concurrent workers (default: 3)"
    )

    parser.add_argument(
        "--runs-dir", type=str, help="Custom autonomous runs directory (default: from settings)"
    )
