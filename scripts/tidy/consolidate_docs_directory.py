...
def main():
    parser = argparse.ArgumentParser(description="Consolidate documentation for a specific directory")
    parser.add_argument("--directory", required=True, help="Directory to consolidate (relative to project root)")
    parser.add_argument("--dry-run", action="store_true", help="Preview without making changes")
    parser.add_argument("--project", default="Autopack", help="Project directory name")
    args = parser.parse_args()

    # Resolve paths
    project_dir = REPO_ROOT if args.project == "Autopack" else REPO_ROOT / ".autonomous_runs" / args.project
    ...
    consolidator.directory_specific_mode = True  # Only process files in this directory, not recursively
