...
from .memory_patterns import ProjectNamespaceError  # noqa: F401 - Re-exported for public API
...
# IMP-MAINT-005: Freshness logic extracted to freshness_filter.py
# ---------------------------------------------------------------------------

# Backward compatibility aliases for functions moved to freshness_filter.py
_parse_timestamp = parse_timestamp
_is_fresh = is_fresh
_calculate_age_hours = calculate_age_hours
_calculate_confidence = calculate_confidence
_enrich_with_metadata = enrich_with_metadata

# Note: The following are now imported from freshness_filter.py:
# - DEFAULT_MEMORY_FRESHNESS_HOURS (imported as _FF_DEFAULT_FRESHNESS_HOURS)
# - LOW_CONFIDENCE_THRESHOLD, MEDIUM_CONFIDENCE_THRESHOLD
# - FRESH_AGE_HOURS, STALE_AGE_HOURS
# - ContextMetadata dataclass
# - COLLECTION_FRESHNESS_HOURS, get_freshness_threshold

# Re-export the constant under the original name for backward compatibility
DEFAULT_MEMORY_FRESHNESS_HOURS = _FF_DEFAULT_FRESHNESS_HOURS
...
# IMP-MAINT-005: Freshness thresholds now imported from freshness_filter.py
# Note: COLLECTION_FRESHNESS_HOURS and get_freshness_threshold are imported above
# ---------------------------------------------------------------------------
...
class MemoryService:
    ...
    def __init__(
        ...
        # IMP-MAINT-005: Initialize freshness filter (must happen before early return)
        self._freshness_filter = FreshnessFilter(DEFAULT_MEMORY_FRESHNESS_HOURS)

        if not self.enabled:
            self.store = NullStore()
            self.backend = "disabled"
            # IMP-MAINT-005: Initialize vector ops even when disabled
            self._vector_ops = VectorStoreOperations(self.store, self.enabled)
        ...
        # IMP-MAINT-005: Initialize vector store operations wrapper
        self._vector_ops = VectorStoreOperations(self.store, self.enabled)
