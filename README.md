...
- `docs/P0_RELIABILITY_DECISIONS.md` (P0 reliability track - completed 2026-01-04)
...
- **Parallel execution in the most efficient state (safe + isolated)**
  ...
  - **Terminology (intentional)**:
    - **Concurrent runs** = the capability: multiple Autopack runs active at once without interfering (isolation is mandatory).
    - **Parallelism** = the product goal: safe concurrent runs *plus* bounded scheduling, isolation, and deterministic throughput (worktrees + leases + caps).
    - In docs and code, prefer **“parallelism”** when referring to the North Star architecture, and **“concurrent runs”** when referring to the runtime fact (N runs active).
...
### Database intention (reusability-first)
- **Production canonical DB**: **PostgreSQL** (the long-term supported/validated production target).
- **Local/dev default**: **SQLite** (`autopack.db`) for quick start, low-friction iteration, and test isolation.
- **Rule**: Avoid creating “two truths” by letting SQLite and Postgres drift. Schema changes should be versioned and validated against Postgres as the canonical target; SQLite remains a convenience layer for local runs and CI.
...
- See [docs/P0_RELIABILITY_DECISIONS.md](docs/P0_RELIABILITY_DECISIONS.md) for complete P0 implementation + [docs/BUILD_HISTORY.md#p0-beyond](docs/BUILD_HISTORY.md#p0-reliability-track-beyond-p0-contract-test-reshaping) for beyond-P0 polish

#### 2026-01-03: BUILD-170 - SOT Ledgers as Canonical Truth: CI Enforcement & Competing Generator Removal ✅
**Established Single Source of Truth with Automated Drift Detection**
- Added --check mode to sot_summary_refresh.py for CI drift detection (exit 0 clean, exit 1 drift)
- Implemented timestamp normalization to prevent spurious drift from Last_Updated changes
- Created doc format contract tests (README summary validation, BUILD_HISTORY INDEX sync, DEC-### uniqueness)
- Added docs-sot-integrity CI job: pytest tests/docs/ + sot_summary_refresh.py --check + nav-mode doc links
- Created weekly deep doc link scan workflow (report-only, JSON+Markdown artifacts, 90-day retention)
- Removed competing generators from db_sync.py (eliminated "two truths" problem)
- All CI checks passing: Zero drift detected, 3 doc contract tests pass, nav-mode clean
- See [docs/BUILD_HISTORY.md#build-170](docs/BUILD_HISTORY.md#build-170) for details
...
- See [docs/BUILD_HISTORY.md#build-169](docs/BUILD_HISTORY.md#build-169) for details
...
- See [docs/BUILD_HISTORY.md#build-168](docs/BUILD_HISTORY.md#build-168) for details
...
<!-- SOT_SUMMARY_START -->
**Last Updated**: 2026-01-04 01:53

- **Builds Completed**: 180 (includes multi-phase builds, 157 unique)
