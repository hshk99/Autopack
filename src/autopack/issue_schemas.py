...
class ProjectBacklogEntry(BaseModel):
    ...
    first_seen_run_id: Optional[str] = Field(
        None, description="First run where this issue appeared"
    )
    last_seen_run_id: str
    last_seen_at: datetime
    seen_in_tiers: List[str] = Field(
        default_factory=list, description="List of tier_ids where issue occurred"
    )
